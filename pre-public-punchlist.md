...
| # | Item | Status | Notes |
|---|---|---|---|
| 1.1 | **CRITICAL: default network-bind + bearer-transport posture.** | 🔵 | **Decided: Cloudflare Tunnel by default.** Broker built + deployed by Simon (`2200-tunnel-broker.twentytwohundred.workers.dev`, D1 + Worker). Demo LIVE at `https://doug.2200.ai`. **valkyrie flipped to loopback-only (2026-07-01) and verified** ... web on `127.0.0.1:2200`, tunnel serves 200, agents healthy, `runtime.env` backed up. Box-side broker client done + tested on branch `feat/tunnel-broker-client` (byte-compat signing vs Simon's `signInstallToken`). **Remaining box-side (access-mode picker, sealed token store, `cloudflared` sidecar) + the hardening model PARKED pending Doug's design calls** (sacrificial domain for user tunnels? CF Access friction level? per-box SCUT + attestation before public self-serve). Full tin-foil-hat design captured in the 2026-07-01 conversation; write up as a decision record when Doug lands the two open calls. |
