...
Surface confidence is uneven and the runbook must reflect it. Custom connectors registered at grok.com/connectors are confirmed working today on the Grok web and mobile app surfaces — that is the Phase 1 primary path. The Tesla in-car voice surface runs on the same Voice Agent API backend and is expected to work, but consumer-car routing to arbitrary user-registered connectors is not yet strongly confirmed by end-user reports; the in-car experience appears to use a more curated tool subset and is still maturing via OTA. The car is therefore a verify-on-your-hardware target, not a shipped claim. The runbook says exactly that.
...
We will use a **recent-activity + standing-brief** combination:
...
- A user can talk to Grok for an extended period (web and app confirmed; in-car expected and verified per setup), feed research into the fleet, and later find a clear, reviewable package in the Inbox.
...
- Expanded tool surface, gated by the rule below
- Improved in-car connector management experience

**Second-factor rule (locked principle):** No execution verb is exposed through the tunnel in Phase 1, so the bearer token is sufficient. Any future tool with external effects — task execution, Agent creation, or anything that spends, sends, or deletes — requires authentication beyond the bearer-at-the-door before it can ever be exposed. This is the standing answer to any later proposal to expose `submit_task` or equivalent: not without a second factor.
