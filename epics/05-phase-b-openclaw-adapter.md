...
**v0.3 ... zero-flag first-run detection SHIPPED 2026-06-13 in `2026.613.1445`** (PR #277, Doug's "ask IF detected, not blank users, no flag" requirement). The installer detects `~/.openclaw` and seeds the banner (non-TTY safe); the bare-`2200` first-run wizard then offers migration automatically, ONLY when OpenClaw is present, no flag. Runs through the daemon's `cli.build.from-handoff` RPC (no migrate-vs-daemon state-file race), copies LLM keys, prints report + checklist + disable guidance; every failure path non-fatal. Security hardening from the adversarial review landed with it: `collectOpenClawLlmEnv` is now an explicit LLM-provider allowlist (no longer copies `GITHUB_TOKEN`/AWS/Stripe/channel tokens), and `upsertRuntimeEnvKey` enforces 0600 on every write. **Now-remaining:** same-host automated disable, Discord-token vault carry, opt-in transcript import, and the post-install "migration assistant after-the-fact" Doug named (a re-runnable surface for users who skipped the first-run offer).
