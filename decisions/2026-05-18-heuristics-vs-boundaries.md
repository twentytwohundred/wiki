...
- **Capability publisher trust contract** (Phase F §1). v1 ships the schema field; M3 enforcement (Epic 17 *External Capability Publishers*) converts the declared field into a structural boundary. Today's state is closer to a heuristic than a boundary; the doc is honest about that transition.
...
- **The combined story:** *"We give you four structural boundaries, plus the same heuristic layers everyone has, and we're honest that OS-level isolation is the operator's responsibility ... with concrete guidance on Docker/Modal/SSH for the OS-level piece."* Competitive framing is implicit in the contrast; making it explicit invites a fight on whose-boundaries-count, which is not the fight we want. The honesty is the story.
