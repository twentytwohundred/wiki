...
**2026-06 adds (sessions toward first external users):** CalVer releases on npm (`@twentytwohundred/2200-cli`, scheme [[decisions/2026-06-12-calver-versioning]]). **Web search** as a configurable BYO-key tool (Brave default + Gemini grounding + Google, hot-reloaded, carried on migration ... [[decisions/2026-06-16-web-search-byo-key]]). **OpenClaw migration** now vaults *every* credential, not just LLM/search keys ([[decisions/2026-06-16-secrets-to-vault]]). **Chat connectors** brought to parity: Discord, Telegram, and Slack are full per-Agent connectors that ship in npm ([[design/connectors/README]], [[decisions/2026-06-16-connectors-dependency-free-gateways]]); WhatsApp remains dev-only (Baileys doesn't bundle). **First-run** now offers a local / self-hosted model (Ollama / LM Studio / vLLM, key optional) alongside cloud keys and Grok sign-in, and **conversational onboarding** was hardened so it never 500s on a model hiccup and extracts cadence→cron + integration gaps. **The Studio** went through a stability pass: the bundled OpenPub pub-server is patched (overlaid onto npm installs) so Agents stop being dropped ~60s after joining, the Bartender is kept off (no third persona, no LLM credential), each Agent shows once by its real name, chat history persists on-box (`state/openpub/<pub>/messages.jsonl`), and the ambient router tolerates the SuperGrok concurrency limit (plus `@all`/`@everyone` broadcast) ... see [[decisions/2026-06-17-pub-server-keepalive-overlay-no-bartender]] and [[decisions/2026-06-17-pub-registration-idempotency-and-operator-name]]. The web app now actually ships in the npm package and serves over LAN/Tailscale. Current release `2026.617.1412`; full delta in [[state/2026-06-18]] and [[handoffs/hobby/2026-06-17]].
