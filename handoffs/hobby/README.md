...
| 2026-04-29 (s4) | [[2026-04-29-session-4]] | Eight PRs (#95-#102) shipped: CLI fix; Epic 15 Phase A complete (Agent detail + start/stop endpoints, Inbox + respond/dismiss endpoints, ⌘K command palette); Epic 8 Phase B (shared brain); Epic 10 Phase A (model catalog); Epic 12 Phase A (extensions framework substrate); Epic 11 Phase A (skills ingestion). 15 of 19 epics with shipped phases on main. 911 runtime tests + 64 web tests = 975 total. |
| 2026-04-29 (s5) | [[2026-04-29-session-5]] | Two PRs (#103, #104). #103 fixed the long-standing flaky scheduler test (microtask race in the test, not the production code; 5 consecutive clean runs after fix). Per-epic specs written for 8B/10/11/12 + Phase B status update on Epic 8 spec. #104 shipped Epic 9 Phase B substrate: per-Agent encrypted credential vault (AES-256-GCM, HKDF-derived per-Agent wrapping key) + `vault` SecretRef source + `2200 credential set/list/show/delete` CLI with stdin-only set. Setup for OAuth flows in 9B-2. 935 runtime tests + 64 web tests = 999 total. |
